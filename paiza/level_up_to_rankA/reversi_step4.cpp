...
void searchSlant(string str) {
  ...
  if(str == "x-y-") {
    ...
    if(x2 != -1 && y2 != -1) changeSlant(x2 + 1, y2 + 1, sum, "RD"); //RD:右下
    ...
    if(x2 != -1 && y2 != h) changeSlant(x2 + 1, y2 - 1, sum, "RU");
  } else if(str == "x+y-") {
    x2++;
    y2--;
    ...
    if(x2 != w && y2 != -1) changeSlant(x + 1, y - 1, sum, "RU");
  } else if(str == "x+y+") {
    x2++;
    y2++;
    ...
    if(x2 != w && y2 != h) changeSlant(x + 1, y + 1, sum, "RD");
  }
}
