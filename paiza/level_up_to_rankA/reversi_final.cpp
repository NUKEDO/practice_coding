...
void search(string str) {
  ...
  if(str == "y-") {
    y2--;
    while(y2 >= 0 && pic.at(y2).at(x2) != '#' && pic.at(y2).at(x2) != c) y2--;
    if(y2 != -1 && pic.at(y2).at(x2) == c) changePic(y2 + 1, y - 1, "y");
  } else if(str == "y+") {
    y2++;
    while(y2 < h && pic.at(y2).at(x2) != '#' && pic.at(y2).at(x2) != c) y2++;
    if(y2 != h && pic.at(y2).at(x2) == c) changePic(y + 1, y2 - 1, "y");
  } else if(str == "x-") {
    x2--;
    while(x2 >= 0 && pic.at(y2).at(x2) != '#' && pic.at(y2).at(x2) != c) x2--;
    if(x2 != -1 && pic.at(y2).at(x2) == c) changePic(x2 + 1, x - 1, "x");
  } else if(str == "x+") {
    x2++;
    while(x2 < w && pic.at(y2).at(x2) != '#' && pic.at(y2).at(x2) != c) x2++;
...
void searchSlant(string str) {
  ...
  if(str == "x-y-") {
    ...
    y2--;
    while(x2 >= 0 && y2 >= 0 && 
          pic.at(y2).at(x2) != '#' && pic.at(y2).at(x2) != c) {
      sum++;
      x2--;
      y2--;
    }
